...
@router.get("/drafts")
def list_drafts(db: Session = Depends(get_db), user: CurrentUser = Depends(get_current_user)):
    return success(ReportService(db).list_drafts(user))


@router.get("/drafts/{draft_id}")
def get_draft(draft_id: int, db: Session = Depends(get_db), user: CurrentUser = Depends(get_current_user)):
    return success(ReportService(db).get_draft(draft_id, user))


@router.post("/drafts/{draft_id}/confirm")
def confirm_draft(draft_id: int, db: Session = Depends(get_db), user: CurrentUser = Depends(get_current_user)):
...
def publish_report(report_id: int, db: Session = Depends(get_db), user: CurrentUser = Depends(get_current_user)):
