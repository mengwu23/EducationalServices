...
如果用户的输入与以下 6 个业务操作完全无关（如问候语、闲聊、天气、非业务话题），则返回：
```json
{{"intent": "irrelevant", "parameters": {{}}}}
```

6 个业务操作是：新增客户、更新客户状态、提交日报、录入成绩、请假审批、投诉处理。
如果输入与其中任何一个相关，请正常解析；只有完全无关时才返回 irrelevant。
...
_REPORT_EXTRACT_PROMPT_TPL = """你是一个日报结构化提取引擎。
从员工口述的日报内容中提取结构化字段。
...
提取以下 4 个字段，如果原文中没有相关信息则填空字符串：

1. summary: 对今日工作内容的一句话总结摘要（50字以内）
2. key_progress: 关键进展（提取具体成果、签约、跟进等）
3. risks: 风险与问题（提取风险、问题、疑虑等，没有则填空字符串）
4. tomorrow_plan: 明日计划（提取明天/明日/下一步计划，没有则填空字符串）
...
class OperationLlmClient:
    ...
    FIELD_PATTERNS: Dict[str, List[str]] = {
        ...
        "exam_type": [rf"(?:考试类型|exam){_ANY_SEP}{_VAL}",
                       r"(模考|期中|期末|月考|周考|daily|midterm|final|补考|平时)(?:\s*成绩|\s*考试)?"],
        "semester": [rf"(?:学期|学段){_ANY_SEP}([^，。；]{{2,15}})"],
        "exam_date": [rf"(?:考试日期|日期){_ANY_SEP}(\d{{4}}-\d{{1,2}}-\d{{1,2}})",
                       r"(\d{4}-\d{1,2}-\d{1,2})"],
        "remark": [rf"(?:备注|说明|附言){_ANY_SEP}{_VAL}",
                   r"成绩\s*([^，。；]{2,30})$",
                   r"([^，。；]{2,20})分[，。；]\s*成绩\s*([^，。；]{2,30})"],
    }
    ...
    def extract_daily_report_fields(self, raw_content: str) -> Dict[str, str]:
        """从日报原始内容中提取结构化字段（summary, key_progress, risks, tomorrow_plan）。

        使用 DeepSeek 做二次提取；不可用时返回空字典。
        """
        if not self._deepseek:
            return {}
        from ..core.config import get_settings
        model = get_settings().deepseek_model or "deepseek-chat"
        prompt = _REPORT_EXTRACT_PROMPT_TPL.format(raw_content=raw_content)
        try:
            response = self._deepseek.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": "你是一个日报结构化提取助手。"},
                    {"role": "user", "content": prompt},
                ],
                temperature=0.05,
                max_tokens=512,
    ...
    def _extract_fields(self, query: str, schema_keys: List[str]) -> Dict[str, Any]:
        ...
        if "student_name" in schema_keys and "student_name" not in params:
            ...
            if nm:
                ...
                # 兜底：XXX录入成绩 / XXX的成绩
                nm = re.search(r"(\S{2,6})录入\s*(?:成绩|分数)", query)
                if nm:
                    params["student_name"] = nm.group(1).strip()

        # 特殊处理：从"同意/驳回 XXX" 提取请假学生姓名
        if "student_name" in schema_keys and "student_name" not in params:
            for prefix in ["同意", "驳回", "拒绝", "通过", "批准"]:
                fm = re.search(prefix + r"\s*(\S{1,6}?)(?:的|请假|$)", query)
                if fm and fm.group(1).strip():
                    params["student_name"] = fm.group(1).strip()
                    break

        # 特殊处理：从"把/将XXX的投诉"提取投诉学生名
        if "student_name" in schema_keys and "student_name" not in params:
            cm = re.search(r"[把将]\s*(\S{1,6}?)(?:的投诉|的反馈|投诉)", query)
            if cm and cm.group(1).strip():
                params["student_name"] = cm.group(1).strip()
    ...
    def _extract_multi_scores(self, query: str) -> List[Dict[str, Any]]:
        results = []
        pattern1 = rf"(?:{'|'.join(self.SCORE_COURSE_KEYWORDS)})\S*?\s*(\d+(?:\.\d+)?)\s*分"
